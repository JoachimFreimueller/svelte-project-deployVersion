<script>
    ...
    onMount(async () => {
        try {
            ...
            if (!response.ok) {
                throw new Error(
                    `Fehler beim Laden der Produkte: ${response.status}`,
                );
            }
...
<div class="p-8 bg-gray-50 min-h-screen">
    ...
    {#if errorOccured}
        <div
            class="bg-red-100 border border-red-400 text-red-700 px-6 py-4 rounded-xl mb-6 text-sm sm:text-base shadow-sm"
        >
